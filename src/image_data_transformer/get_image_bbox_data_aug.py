...
def rotate_bbox_labels(bbox, angle, img_width, img_height):
    """Rotate YOLO bounding box coordinates"""
    ...
    if angle == 90:
        ...
        rotated_corners = [(img_width -y,  x) for x, y in corners]
    elif angle == 180:
        # 180 degrees: (x,y) -> (width-x, height-y)
        corners = [(x1, y1), (x2, y1), (x2, y2), (x1, y2)]
        rotated_corners = [(img_width - x, img_height - y) for x, y in corners]
    elif angle == 270:
        # 270 degrees clockwise: (x,y) -> (height-y, x)
        corners = [(x1, y1), (x2, y1), (x2, y2), (x1, y2)]
        rotated_corners = [(y, img_height - x) for x, y in corners]
    else:
        return bbox
    ...
    # Check if bbox is still valid after rotation
...
def parse_yolo_bbox(line):
    """Parse YOLO bounding box line"""
...
def format_yolo_bbox(bbox):
    """Format bounding box back to YOLO format"""
    class_id, x_center, y_center, width, height = bbox
    return f"{class_id} {x_center:.6f} {y_center:.6f} {width:.6f} {height:.6f}"

def copy_original_image_and_labels(image_path, label_path, output_dir):
    """Copy original image and labels to output directory"""
...
def rotate_image_and_bbox(image_path, label_path, output_dir, angle):
    """Rotate image and its bounding box labels"""
    # Read image
    ...
    if os.path.exists(label_path):
        ...
        for line in lines:
            if line.strip():
                ...
                if bbox is not None:
                    # Rotate bbox (input is already normalized)
                    rotated_bbox = rotate_bbox_labels(bbox, angle, width, height)
                    
                    # Only add valid bboxes
                    if rotated_bbox is not None:
                        # Convert back to normalized coordinates for the NEW image dimensions
                        normalized_bbox = [
                            rotated_bbox[0],  # class_id
                            rotated_bbox[1] / new_width,   # x_center
                            rotated_bbox[2] / new_height,  # y_center
                            rotated_bbox[3] / new_width,   # width
                            rotated_bbox[4] / new_height   # height
                        ]
                        
                        # Format and add to labels
                        rotated_labels.append(format_yolo_bbox(normalized_bbox))
...
def process_bbox_aug(zip_file, input_path, angles, keep_original, progress=gr.Progress(track_tqdm=True)):
    """Process YOLO data (ZIP or path) for bounding box augmentation and return results"""
    ...
    try:
        ...
        pbar = progress.tqdm(image_files, desc="Processing bounding box augmentation")
        ...
        for idx, img_path in enumerate(pbar, 1):
            ...
            try:
                ...
                # Apply rotations
                for angle in angles:
                    rotate_image_and_bbox(str(img_path), label_path, temp_dir, angle)
        ...
        zip_path = os.path.join(temp_dir, "bbox_augmented.zip")
